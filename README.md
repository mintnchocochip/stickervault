...
WhatsApp also documents a limit of 1 to 10 packs per app (~300 stickers). Whether
it is enforced at runtime is still unverified.
...
| Filter bypass via control characters | Control characters and over-long names are rejected *before* pattern matching, because `$` matches before a trailing newline in most regex flavours |
